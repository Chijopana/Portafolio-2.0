...
<div
  className={`hidden md:flex fixed z-50 flex-col left-4 top-1/2 -translate-y-1/2 gap-6 ${
...
>
  ...
  <a href="https://www.instagram.com/joseblondel1" target="_blank" rel="noopener noreferrer" className="hover:text-pink-500 transition-colors duration-300">
    <FiInstagram size={24} />
      ...
      <header className="flex items-center justify-between py-6 mb-20 max-w-7xl mx-auto px-4">
        <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className={`text-6xl font-extrabold tracking-wide flex-grow max-w-4xl drop-shadow-lg ${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>{t('greeting')}</motion.h1>





        <motion.button onClick={() => setDarkMode(!darkMode)} whileHover={{ scale: 1.1, boxShadow: "0 0 8px rgba(59, 130, 246, 0.6)" }} className={`ml-6 p-2 rounded-md transition-colors duration-300 shadow-md ${darkMode ? 'bg-gray-700 text-yellow-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`} aria-label="Toggle dark mode">
          {darkMode ? <FiSun size={24} /> : <FiMoon size={24} />}
        </motion.button>



{/* Botones de idioma adaptativos */}
<div className="ml-8 relative">
  {/* Escritorio: botones individuales */}
  ...
  {/* Móvil: un solo botón desplegable */}
